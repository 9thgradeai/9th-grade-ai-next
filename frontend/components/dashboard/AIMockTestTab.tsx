...
export default function AIMockTestTab() {
  ...
  return (
    <div className="mx-auto flex w-full max-w-3xl flex-col gap-5 px-4 py-6">
      <div>
        <h1 className="text-xl font-semibold text-zinc-100">AI মক টেস্ট</h1>
      ...
      <div className="flex flex-wrap items-end gap-3 rounded-2xl border border-white/10 bg-zinc-900/60 p-4">
        <label className="flex flex-col gap-1 text-sm text-zinc-300">
          ...
          <input
            ...
            className="w-40 rounded-lg border border-white/10 bg-zinc-900 px-3 py-2 text-sm text-zinc-100 outline-none focus:border-emerald-500/50"
          />
        </label>
        <label className="flex flex-col gap-1 text-sm text-zinc-300">
          প্রশ্ন সংখ্যা
          <input
            ...
            className="w-24 rounded-lg border border-white/10 bg-zinc-900 px-3 py-2 text-sm text-zinc-100 outline-none focus:border-emerald-500/50"
          />
        </label>
        <label className="flex flex-col gap-1 text-sm text-zinc-300">
          কঠিনতা
          <select
            ...
            className="w-32 rounded-lg border border-white/10 bg-zinc-900 px-3 py-2 text-sm text-zinc-100 outline-none focus:border-emerald-500/50"
          >
        ...
        <button
          ...
          disabled={loading}
          className="rounded-xl bg-emerald-500 px-5 py-2 text-sm font-medium text-zinc-950 transition-colors hover:bg-emerald-400 disabled:opacity-60"
        >
      ...
      {test && test.questions.length > 0 && (
        <>
          <div className="flex items-center justify-between rounded-xl border border-white/10 bg-zinc-900/60 px-4 py-3">
            <div>
              <h2 className="font-semibold text-zinc-100">{test.title}</h2>
              <p className="text-xs text-zinc-400">সূত্র: {test.source}</p>
            </div>
            {submitted && (
          ...
          {!submitted ? (
            <button
              ...
              className="self-start rounded-xl bg-emerald-500 px-5 py-2 text-sm font-medium text-zinc-950 hover:bg-emerald-400"
            >
              দাখিল করো
            </button>
          ) : (
            <button
              ...
              className="self-start rounded-xl border border-white/10 px-5 py-2 text-sm text-zinc-200 hover:border-emerald-500/50"
            >
              আবার চেষ্টা করো
            </button>
          )}
      ...
      {test && test.questions.length === 0 && !loading && (
        <p className="text-sm text-zinc-400">কোনো প্রশ্ন তৈরি হয়নি। আবার চেষ্টা করো।</p>
      )}
    </div>
  );
}
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-zinc-900/60 p-4">
      <div className="mb-3 flex items-start gap-2">
        <span className="mt-0.5 font-mono text-xs text-zinc-500">{index}.</span>
        <div>
          <p className="text-sm text-zinc-100">{q.question}</p>
          {q.topic && <span className="text-[11px] text-zinc-500">{q.topic}</span>}
        </div>
      </div>
        ...
        {q.options.map((opt) => {
          ...
          const cls = showAnswer
            ...
            : isSelected
              ...
              : "border-white/10 text-zinc-300 hover:border-emerald-500/30";
          return (
            <button
              key={opt.id}
              type="button"
      ...
      {showAnswer && q.explanation && (
        <p className="mt-3 rounded-lg bg-zinc-800/60 px-3 py-2 text-xs text-zinc-300">
          {q.explanation}
        </p>
      )}
    </div>
