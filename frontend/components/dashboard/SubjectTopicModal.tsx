 ...
 *
 * Layout:
 *   • Mobile  : full-viewport bottom sheet — subjects as scrollable list,
 *               tapping a subject shows its topic tree (drill-down pattern),
...
import { X, Check, ChevronRight, ChevronLeft, Minus, Plus, Search, Zap } from "lucide-react";
...
export default function SubjectTopicModal({
  ...
  useEffect(() => {
    if (subjects.length > 0 && activeSubjectId === null) {
      ...
      queueMicrotask(() => setActiveSubjectId(firstId));
    }
    ...
    <>
      ...
      >
        ...
        >
          {/* ── HEADER ────────────────────────────────────────── */}
          <div
            className="flex items-center justify-between px-4 py-3 sm:px-5 sm:py-4 border-b flex-shrink-0"
          ...
          <div className="flex flex-1 min-h-0" style={{ overflow: "hidden" }}>
            ...
            <div className="flex flex-col flex-1 min-w-0 sm:hidden">
              ...
              <div
                className="flex-shrink-0 flex gap-2 px-3 py-2.5 border-b overflow-x-auto scrollbar-none"
                style={{ borderColor: "var(--dashboard-border-muted)", background: "var(--dashboard-surface-muted)" }}
              >
                {subjects.map((subject) => {
                  ...
                  return (
                    <button
                      ...
                      className="flex-shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold transition-all min-h-[36px]"
                      style={
                        isActive
                    ...
                    >
                      ...
                      <span className="truncate max-w-[80px]">{subject.nameBn}</span>
              ...
              {/* Mobile: active subject header + select all */}
              {activeSubject && (
                <div
                ...
                >
                  <div className="flex items-center gap-2 min-w-0">
                    <span className="text-lg flex-shrink-0">{activeSubject.icon}</span>
                    <div className="min-w-0">
              ...
              {/* Mobile: topic tree — scrollable */}
              <div className="flex-1 overflow-y-auto px-3 py-2 space-y-0.5">
                {activeSubject && selection[activeSubject.id] !== undefined ? (
                  ...
                  <div className="py-10 text-center">
                    <p className="text-sm font-mono" style={{ color: "var(--dashboard-text-muted)" }}>
                      উপরের বার থেকে একটি বিষয় বাছাই করুন
                    </p>
                  </div>
                )}
              </div>

              {/* Mobile: per-subject count stepper */}
              {activeSubject && selection[activeSubject.id] !== undefined && (
                <div
                  className="flex-shrink-0 flex items-center justify-between gap-3 px-4 py-2.5 border-t"
                  style={{ borderColor: "var(--dashboard-border-muted)", background: "var(--dashboard-surface-muted)" }}
                >
                  <div className="min-w-0">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="w-9 h-9 rounded-xl flex items-center justify-center transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--dashboard-primary)]"
            ...
            {/* Desktop: right pane — topic tree */}
            <div className="hidden sm:flex sm:flex-col flex-1 min-w-0" style={{ overflow: "hidden" }}>
              {activeSubject ? (
                <>
          ...
          {/* ── FOOTER ──────────────────────────────────────────── */}
          <div
            className="flex-shrink-0 flex items-center justify-between gap-3 px-4 py-3 sm:px-5 sm:py-4 border-t"
            style={{ borderColor: "var(--dashboard-border-muted)", background: "var(--dashboard-surface-muted)" }}
          >
            <div className="min-w-0">
              {totalCount > 0 ? (
