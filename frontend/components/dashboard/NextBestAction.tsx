...
const INTENSITY_STYLE: Record<NextAction["intensity"], React.CSSProperties> = {
  high: { background: "var(--dashboard-primary)", color: "white", borderColor: "var(--dashboard-primary)" },
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-start gap-3.5">
          <div
            ...
            style={
              isHigh
                ? { background: "rgba(255,255,255,0.16)", borderColor: "rgba(255,255,255,0.2)", color: "white" }
          ...
          <div>
            <p className="mb-1 text-[11px] font-semibold uppercase tracking-[0.12em]" style={{ color: isHigh ? "rgba(255,255,255,0.85)" : "var(--dashboard-primary)" }}>
              পরবর্তী সেরা পদক্ষেপ
            </p>
            <h2 className="text-balance text-[17px] font-semibold leading-tight" style={{ color: isHigh ? "white" : "var(--dashboard-text-primary)" }}>{action.title}</h2>
            <p className="mt-1.5 leading-relaxed text-[13px]" style={{ color: isHigh ? "rgba(255,255,255,0.88)" : "var(--dashboard-text-secondary)" }}>{action.description}</p>
          </div>
        </div>
        <button
          type="button"
          ...
          style={
            isHigh
              ? { background: "white", color: "var(--dashboard-primary)" }
