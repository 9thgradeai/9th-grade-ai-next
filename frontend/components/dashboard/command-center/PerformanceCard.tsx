...
export default function PerformanceCard({ activity, results }: { activity: ActivityPoint[]; results: { score: number }[] }) {
  const [range, setRange] = useState<"7D" | "30D" | "ALL">("7D");
  const points = useMemo(() => {
    if (range === "7D") return activity.slice(-7);
    if (range === "30D") return activity.slice(-30);
    return activity;
  }, [activity, range]);

  const max = Math.max(1, ...points.map((p) => p.answered));
    ...
    <div className="command-card p-5 sm:p-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-xs mt-1" style={{ color: "var(--dashboard-text-muted)" }}>{totalSolved} solved · {accuracy}% accuracy · consistency reflects streak</p>
        </div>
        <div className="flex items-center rounded-full border p-1 gap-1" style={{ borderColor: "var(--dashboard-border-muted)", background: "var(--dashboard-surface-muted)" }}>
          {(["7D", "30D", "ALL"] as const).map((r) => (
            <button key={r} onClick={() => setRange(r)} className="px-3 py-1 rounded-full text-xs font-bold transition-colors" style={range === r ? { background: "var(--dashboard-primary)", color: "var(--dashboard-text-inverse)" } : { color: "var(--dashboard-text-muted)" }}>{r}</button>
          ))}
        </div>
      </div>
      ...
      <div className="mt-5 rounded-xl border p-3" style={{ borderColor: "var(--dashboard-border-muted)", background: "var(--dashboard-surface-muted)" }}>
        ...
        <div className="flex justify-between text-[10px] font-medium mt-1" style={{ color: "var(--dashboard-text-muted)" }}>
          {points.map((p, i) => (
            <span key={i}>{new Date(p.date).toLocaleDateString("en-GB", { day: "2-digit", month: "short" })}</span>
          ))}
        </div>
      </div>
      ...
      <div className="mt-4 grid grid-cols-3 gap-2">
        ...
        <div className="rounded-xl border px-3 py-2.5" style={{ background: "var(--dashboard-surface)", borderColor: "var(--dashboard-border-muted)" }}>
          <p className="text-[10px] font-bold uppercase tracking-widest flex items-center gap-1" style={{ color: "var(--dashboard-text-muted)" }}><Activity className="w-3 h-3" /> Efficiency</p>
