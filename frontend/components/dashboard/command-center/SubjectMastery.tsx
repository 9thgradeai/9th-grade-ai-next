    ...
    <div className="command-card p-5 sm:p-6">
      <div className="flex items-end justify-between gap-4">
        <div>
          ...
          <p className="text-xs mt-1" style={{ color: "var(--dashboard-text-muted)" }}>Tap any subject to start a focused practice</p>
        </div>
      ...
      {display.length === 0 ? (
        ...
        <div className="mt-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {display.map((r) => {
            ...
            const isWeak = pct < 65;
            return (
              <button key={r.name} onClick={() => onPractice(r.name)} className="text-left rounded-2xl border p-4 flex flex-col gap-3 transition-transform hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--dashboard-focus-ring)]" style={{ background: isWeak ? "var(--dashboard-danger-subtle)" : "var(--dashboard-surface-muted)", borderColor: isWeak ? "color-mix(in srgb, var(--dashboard-danger) 16%, var(--dashboard-border-muted))" : "var(--dashboard-border-muted)" }}>
                ...
                <span className="text-[11px] font-semibold inline-flex items-center gap-1" style={{ color: "var(--dashboard-primary)" }}>Practice →</span>
              </button>
            );
          })}
        </div>
