...
// Now includes tracing, metrics, circuit breaker integration,
// duplicate-tool-call detection, and checkpoint-based resume.

import "server-only";

import { resolveCandidatesForModelTask, reportProviderOutcome } from "../providers";
...
import { saveCheckpoint, getLatestCheckpoint, clearCheckpoints } from "./checkpoint";
import { AppError } from "~backend/errors";
import { createTraceContext, traced } from "../infrastructure/tracing";
import { recordAgentStep } from "../infrastructure/metrics";
...
export async function runAgentTurn(opts: {
  ...
  // Create trace context for this agent turn
  const trace = createTraceContext("agent.turn", {
    userId,
    intent,
    questionLength: question.length,
  });

  const context = await traced(trace, "context.build", async () => {
    return buildContext({
      userId,
      task: "assistant",
      intent,
  ...
  trace.event("run.created", "ok", { runId });
  ...
  // Track tool calls to detect duplicate consecutive calls
  ...
  try {
    ...
    for (const cand of candidates) {
      ...
      try {
        while (steps < MAX_AGENT_STEPS) {
          steps += 1;
          recordAgentStep(providerUsed, steps, false);

          const raw = await traced(trace, `step.${steps}.model`, async (span) => {
            span.metadata.provider = providerUsed;
            span.metadata.model = modelUsed;
            return runStep(provider, history, system);
          });
          ...
          if (turn.toolCall) {
            ...
            // Detect duplicate consecutive tool calls (loop protection)
            const lastTwo = recentToolCalls.slice(-2);
            if (lastTwo.length === 2 && lastTwo[0] === name && lastTwo[1] === name) {
              trace.event("agent.duplicate_tool", "error", { tool: name });
              // Force final output by appending a note to history
              history.push(msg);
              history.push({
                role: "user",
                content: `Tool ${name} was called repeatedly. Please provide your final answer using the data you already have.`,
              });
              continue;
            }
            ...
            let result: { ok?: boolean; summary: string; data?: Record<string, unknown> } = { ok: false, summary: "" };
            let success = false;

            await traced(trace, `step.${steps}.tool.${name}`, async (span) => {
              span.metadata.tool = name;
              span.metadata.args = JSON.stringify(argObj).slice(0, 200);
              if (!def) {
            ...
            recordAgentStep(providerUsed, steps, true);

            // Save checkpoint after successful tool call for resume capability
            if (success) {
              saveCheckpoint({
                runId,
                step: steps,
                history: [...history],
                sessionToolResults: { ...sessionToolResults },
          ...
          // Report provider success to circuit breaker
    ...
    trace.finish("error", { errorCode: code });
